...
enum AlbumMergeMode: String, CaseIterable {
    case artist = "Merge Artist"
    case album = "Merge into One Album"
}

struct MergeAlbumsView: View {
    let selectedAlbums: [AlbumInfo]
    var onSave: () -> Void
    ...
    @State private var mergeMode: AlbumMergeMode = .artist
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            Picker("", selection: $mergeMode) {
                ForEach(AlbumMergeMode.allCases, id: \.self) { mode in
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text("\(selectedAlbums.count) albums selected")
                    .font(.system(size: theme.typography.captionSize, weight: .medium))
                    .foregroundStyle(colors.textSecondary)

                ScrollView {
                    VStack(alignment: .leading, spacing: 2) {
                        ForEach(selectedAlbums) { albumInfo in
                            HStack(spacing: 6) {
    ...
    private func prefillDefaults() {
        // Find the most common artist name among selected albums
        var artistCounts: [String: Int] = [:]
        for albumInfo in selectedAlbums {
            if let name = albumInfo.artist?.name {
                artistCounts[name, default: 0] += 1
            }
        }
        let mostCommon = artistCounts.max(by: { $0.value < $1.value })?.key ?? ""
        artistName = mostCommon
        artistFilter = mostCommon

        // For album merge mode, pick the most common album title
        var titleCounts: [String: Int] = [:]
        for albumInfo in selectedAlbums {
            titleCounts[albumInfo.album.title, default: 0] += 1
        }
        albumTitle = titleCounts.max(by: { $0.value < $1.value })?.key ?? selectedAlbums.first?.album.title ?? ""
    }

    private func performMerge() {
        let albumIds = selectedAlbums.compactMap { $0.album.id }
        ...
        do {
            try db.dbPool.write { dbConn in
                switch mergeMode {
                case .artist:
                    try LibraryQueries.mergeAlbumsToArtist(
                        albumIds: albumIds,
                ...
                case .album:
                    ...
                    try LibraryQueries.mergeAlbumsIntoOne(
                        albumIds: albumIds,
                        targetAlbumTitle: trimmedTitle,
                        artistName: trimmedArtist,
                        in: dbConn
                    )
                }
            ...
            print("Failed to merge albums: \(error)")
        }
    }
}
